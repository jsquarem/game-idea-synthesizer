...
- dependency-graph (for initial dependency creation; created links drive the "what systems interact with which and how" view)
...
- Actions: `app/actions/synthesis.actions.ts` (getSynthesisConfig, convertSynthesisAction, addSuggestedSystemsToExtractionAction, renameSynthesisAction, deleteSynthesisAction), `app/actions/system-detail.actions.ts` (create/update/delete system detail on system detail page)
...
- Frontend: **3-step wizard** (Configure → Processing → Review) at `.../brainstorms/[sessionId]/synthesize`; two-column layout (wizard left; right column step-dependent). **Entry:** list click and post-create go directly to synthesize (no session preview page); Idea Stream "Finalize + Synthesize" and session output links `?output=<id>` also land on synthesize. **Configure step** right column: synthesis list for this brainstorm (Load, Rename, Delete); list from listOutputsByBrainstorm; renameSynthesisAction and deleteSynthesisAction. **Processing and Review** right column: markdown preview (extractionToMarkdown). Processing step shows live-shaped skeleton (ExtractionAccordion without selection) that populates as the stream is parsed; Review step Extraction tab: single refine form, then Finalize block (Get AI suggestion, Apply suggestion, Create selected), then extracted systems as independently expandable list (left expand handle, Include/Exclude button per system on far right; header row with Add all/Exclude all; selection as buttons with icons, status as New/Existing badges; consistent iconography). Prompt & raw tab shows prompt and raw output. System detail at `.../systems/[systemId]` includes System details block.
...
- 2026-02-18: Extraction instructions updated for flowchart granularity and interaction-flow dependencies (context-builder); supports systems interaction graph.
- 2026-02-19: Extraction UI shows per-system dependencies (Depends on badges / No dependencies); convert derives edges from synthesis stubs when user does not use Get AI suggestion; Create selected redirects to dependency graph; output-new.json and docs updated.
- 2026-02-19: Graph framing: interaction links drive "what systems interact with which and how"; Review step shows "Interacts with" and "Interaction links" (AI suggestion); convert-suggest supports optional per-link description; Finalize and Interacts-with section updated in Current Implementation.
- 2026-02-19: Gap-based suggested systems: prompt asks AI to suggest new systems when a system would be solo (optional suggestedSystems/suggestedSystemDetails); Suggested systems (fill gaps) section in Review with Suggestion badge and Add to extraction; addSuggestedSystemsToExtractionAction promotes suggestions into extraction and persists; refined systems and convert flow include promoted systems; maxTokens 10240; refine persists and returns suggested; docs updated.
- 2026-02-19: Brainstorm preview removed: entry from list and post-create goes to synthesize; Configure step right column = synthesis list (load, rename, delete); Processing/Review = markdown preview. renameSynthesisAction and deleteSynthesisAction in synthesis.actions; synthesized-output.repository update supports title; code mapping updated.
- 2026-02-19: Load synthesis opens Processing step with that synthesis populated (wizard keyed by output id); Configure step adds optional synthesis name (dynamic default) and synthesis date/time; stream API and SynthesisRunInput accept optional title.
- 2026-02-19: Synthesis snapshot: each SynthesizedOutput stores rawInput (brainstorm content at run time) and fullPrompt; create flow persists both. Loading a synthesis shows that snapshot: source preview = saved raw input, Processing = saved prompt + raw output + parsed extraction. GET synthesis/output returns fullPrompt and rawInput.
